...
def remove_extra_marker(data_str):
    while "#" in data_str:
        ...
        data_str = data_str[:marker_start] + data_str[marker_end + 1 :]
    ...
    with open(channel_file, "r") as fp:
        ...
        for line in fp:
            if line[:6] == "# SCAN":
                ...
                print(
                    "WARNING: It appears this channel file contains no scan "
                    "markers, assuming one scan...",
                    end="",
                    flush=True,
                )
                ...
                if line[0] == "Y":
                    ...
                    headers["X-roi"] = "{0} - {1}".format(
                        headers["X-roi"], line.strip()
                    )
                    ...
                    # We have data before
                    print(
                        "WARNING: Data appeared in this channel file before "
                        "any scan or step marker, ignoring row...",
                        end="",
                        flush=True,
                    )
        ...
        if "Frequency" not in headers:
            print(
                "WARNING: No frequency information, assuming continuous...",
                end="",
                flush=True,
            )
            freq = "continuous"
            ...
            if freq not in ("continuous", "step"):
                print(
                    "WARNING: Unknown or unsupported frequency '{0}', "
        ...
        if freq == "step" and shape == 0:
            for scan_idx in range(len(scan_offsets)):
                ...
                try:
                    read_len = (
                        scan_offsets[scan_idx + 1]
                        - scan_offsets[scan_idx]
                        - len(marker_line)
                    )
                except IndexError:
                    read_len = None
            ...
            for scan_idx in range(len(step_offsets)):
                ...
                for step_idx in range(len(step_offsets[scan_idx])):
                    ...
                    step_value = marker_line[
                        marker_line.find(":") + 1 :
                    ].strip()

                    try:
                        read_len = (
                            step_offsets[scan_idx][step_idx + 1]
                            - cur_pos
                            - len(marker_line)
                        )
                    except IndexError:
                    ...
                    if data_len > 0:
                        local_compress_args = (
                            compress_args if data_len > 1024 else {}
                        )
                        ...
                        h5scan.create_dataset(
                            step_value.strip(), shape=(0, column_count)
                        )
...
def convert_hptdc_file(
    channel_file,
    h5ch,
    compress_args={},
    hptdc_chunk_size=10000,
    hptdc_word_format="raw",
    hptdc_ignore_tables=False,
    **kwargs,
):
...
def convert_legacy_hptdc_file(channel_file, h5ch, compress_args={}, **kwargs):
    ...
    with open(channel_file, "rb") as fp:
        ...
        for step_idx in range(n_steps):
            ...
            if data_len % 16 != 0:
                print(
                    "WARNING: Invalid data string length {0} on step "
                    "{1}, skipping!".format(step_idx, data_len)
                )
                return False

            if data_len > 0:
                local_compress_args = compress_args if data_len > 1024 else {}

                data = numpy.frombuffer(
                    data_str, dtype=HptdcHit, count=(data_len // 16)
                )
                h5ch.create_dataset(
                    str(step_idx),
...
def run(
    ...
    **kwargs,
    ...
    for entry in glob_list:
        ...
        for run in runs:
            if (
                entry.startswith(run.path)
                and entry[: entry.rfind(".")] != run.path
            ):
    ...
    if not runs:
        raise ValueError(
            "No runs with channels found using glob pattern: {0}".format(
                glob_str
            )
        )

    print(
        "Found {0} measurement runs with {1:.1f} channels on average".format(
    ...
    for run in runs:
        print(
            "Starting {0} with {1} channels".format(
                run.root, len(run.channels)
            )
        )
        start_time = time.time()

        out_path = "{0}/{1}.h5".format(
            output_dir, output_format.format(**run._asdict())
        )
        ...
        for channel_file in run.channels:
            convert_file(
                channel_file[len(run.path) + 1 : channel_file.rfind(".")],
                channel_file,
                h5f,
                compress_args,
                **kwargs,
...
def main():
    ...
    cli = argparse.ArgumentParser(
        prog="metro2hdf.py", description="Converts METRO data files to hdf5"
    )
    ...
    hdf_group.add_argument(
        ...
        type=str,
        metavar="name",
        choices=["sec2", "stdio", "core", "family"],
